...
pub fn get_app_data_dir() -> CommandResult<std::path::PathBuf> {
    ...
    std::fs::create_dir_all(&app_data_dir)
        .map_err(|e| CommandError::io_error(format!("Failed to create app data directory: {}", e)))?;
...
pub fn get_thumbnails_dir() -> CommandResult<std::path::PathBuf> {
    ...
    std::fs::create_dir_all(&thumbnails_dir)
        .map_err(|e| CommandError::io_error(format!("Failed to create thumbnails directory: {}", e)))?;
...
mod tests {
    ...
    fn test_get_filename_without_extension() {
        assert_eq!(get_filename_without_extension("video.mp4"), Some("video".to_string()));
        assert_eq!(get_filename_without_extension("my-video.mov"), Some("my-video".to_string()));
        assert_eq!(get_filename_without_extension("video"), Some("video".to_string()));
        assert_eq!(get_filename_without_extension(""), None); // Empty string has no filename
    }

    #[test]
    fn test_get_filename_with_extension() {
        assert_eq!(get_filename_with_extension("video.mp4"), Some("video.mp4".to_string()));
        assert_eq!(get_filename_with_extension("my-video.mov"), Some("my-video.mov".to_string()));
        assert_eq!(get_filename_with_extension("video"), Some("video".to_string()));
